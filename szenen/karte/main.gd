...
@export var platte: Texture2D = preload("res://assets/platten/silber.png")
@export var bild: Texture2D = preload("res://assets/bilder/neutral landmark.webp")
@export var fenster_rahmen: Texture2D = preload("res://assets/fenster/kreis/rahmen.png")
@export var fenster_pfad: Texture2D = preload("res://assets/fenster/kreis/rahmen.png")
@export var fenster_glas: Texture2D = preload("res://assets/fenster/kreis/glas.png")
...
func _init(title = self.title, etikett = self.etikett, tafel = self.tafel, level = self.level, leben = self.leben, angriff = self.angriff, platte = self.platte, fenster_rahmen = self.fenster_rahmen, fenster_pfad = self.fenster_pfad, fenster_glas = self.fenster_glas, bild = self.bild, feld = self.feld) -> void:
	self.title = title
	...
	self.fenster_pfad = fenster_pfad
...
func _ready() -> void:
	...
	get_node('Fenster/Pfad').texture = fenster_pfad
	...
	if ist_im_fokus:
		if Input.is_action_pressed("click"):
			...
			if not global.ist_am_ziehen:
				...
				var neue_scale = Vector2(basis_scale.x*1.1, basis_scale.y)
