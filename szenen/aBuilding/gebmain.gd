...
@export var cardimg_scene: PackedScene = preload("res://szenen/hand/preview.tscn")
...
var cardmenu_path
var bring_ressources = {"Holz" : 30, "Stein" : 30, "Metall" : 30, "Amethyst" : 0, "Gold" : 0}
func _ready():
	set_process(true)
...
func _on_phase_changed(new_phase: int):
	if new_phase == GameStateWorld.Phase.DRAWING and GameStateWorld.current_player == player_number:
		for res in bring_ressources.keys():
			GameState.ressources[res] += self.bring_ressources[res]
